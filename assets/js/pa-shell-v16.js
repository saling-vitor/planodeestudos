...
function contestId(){return window.PLANO_ARQ_CONTEST_CONTEXT?.resolveId?.()||(()=>{const q=new URLSearchParams(location.search).get('contest');if(q)return q;const m=location.hash.match(/^#contest\/([^/?#]+)/);if(m)return decodeURIComponent(m[1]);return localStorage.getItem('planoarq:active-contest:v1')||localStorage.getItem('planoarq:active-contest')||localStorage.getItem('planoarq:activeContest')||''})()}
function contests(){const shared=window.PLANO_ARQ_CONTEST_CONTEXT?.contests?.();if(Array.isArray(shared))return shared;const seed=window.PLANO_ARQ_CONTESTS?.contests||[],local=safeJSON(localStorage.getItem('planoarq:contests:v1'),[]),m=new Map(seed.map(x=>[x.id,{...x}]));local.forEach(x=>m.set(x.id,{...(m.get(x.id)||{}),...x}));return[...m.values()]}
...
function bindSidebar(side){side.querySelectorAll('[data-pa-status="future"]').forEach(b=>b.addEventListener('click',()=>{const key=b.dataset.paNav,item=allItems().find(x=>x.key===key);info(item?.label||'Área indisponível',`${item?.label||'Esta área'} ainda não está disponível nesta versão.`,'Indisponível')}))}
...
function ensureMobileNav(){
 ...
 if(current==='home'){
   document.getElementById('paMobileNav')?.remove();
   document.getElementById('paMobileMore')?.remove();
   return
 }
 if(document.getElementById('paMobileNav'))return;
 const moreActive=!['today','edital','maps'].includes(current);
 document.body.insertAdjacentHTML('beforeend',
   `<nav class="pa-mobile-nav" id="paMobileNav">${mobileItem('today','Hoje')}${mobileItem('edital','Edital')}${mobileItem('maps','Mapas')}<button type="button" class="${moreActive?'active':''}" id="paMobileMoreBtn" aria-expanded="false" aria-controls="paMobileMore" ${moreActive?'aria-current="page"':''}>•••<span>Mais</span></button></nav><div class="pa-mobile-more" id="paMobileMore" aria-label="Mais áreas do concurso"></div>`
 );
 const more=document.getElementById('paMobileMore'),btn=document.getElementById('paMobileMoreBtn');

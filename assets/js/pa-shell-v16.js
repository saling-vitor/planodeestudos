...
function closeDrawer(){document.querySelector('.pa-sidebar')?.classList.remove('open');document.querySelector('.pa-backdrop')?.classList.remove('show')}
function bindDrawer(){document.addEventListener('click',e=>{const btn=e.target.closest('.pa-menu-btn');if(!btn)return;const side=document.querySelector('.pa-sidebar'),back=document.querySelector('.pa-backdrop');if(side){side.classList.add('open');back?.classList.add('show')}});document.addEventListener('click',e=>{if(!e.target.classList.contains('pa-backdrop'))return;closeDrawer()})}
...
function closeCommonMenu(){const m=document.getElementById('paCommonMenu');if(m)m.hidden=true}
...
function initWithSync(){init();setTimeout(()=>{window.PLANO_ARQ_SYNC?.startAutoSync?.();window.PLANO_ARQ_DRIVE?.startAutoSnapshot?.()},0)}

...
const tracked=k=>!!k&&(k.startsWith('planoarq:')||k.startsWith('mindmap_state::'))&&!privateKey(k);
...
function contestKeys(cid){const exact=[`planoarq:planning::${cid}`,`planoarq:generated-plan::${cid}`,`planoarq:session-log::${cid}`,`planoarq:review-activity::${cid}`,`planoarq:file-favorites::${cid}`,`planoarq:material-summary::${cid}`];const namespaces=new Set(materialsForContest(cid).map(m=>m.storageNamespace).filter(Boolean));const dynamic=keys().filter(k=>exact.includes(k)||[...namespaces].some(ns=>k===`mindmap_state::${ns}`)||k.includes(`::${cid}`));return [...new Set(dynamic)].sort()}
...
function inspect(payload){if(!payload||!['planoarq-backup-v1','planoarq-backup-v2'].includes(payload.schema))throw new Error('Formato de backup incompatível');const data=payload.data||payload.keys;if(!data||typeof data!=='object')throw new Error('Backup sem dados');const entries=Object.entries(data).filter(([k])=>tracked(k));if(!entries.length)throw new Error('Nenhum dado do Plano ARQ encontrado');return {entries,scope:payload.scope||'legacy',contestId:payload.contestId||'',exportedAt:payload.exportedAt||'',device:payload.device||null}}

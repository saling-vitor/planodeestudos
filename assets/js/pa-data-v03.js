...
const CONTESTS_KEY='planoarq:contests:v1',EXAM_SCHEMA_PREFIX='planoarq:exam-schema::',CONTEST_FILES_PREFIX='planoarq:contest-files::',CONTEST_MATERIALS_PREFIX='planoarq:contest-materials::',IMPORT_DRAFT_PREFIX='planoarq:contest-import-draft::',RECOVERY_PREFIX='planoarq:recovery::',RECOVERY_LAST_KEY='planoarq:last-recovery:v1',RECOVERY_LIMIT=5;
...
const privateKey=k=>!!k&&(
  ...
  k===RUNTIME_KEY||k==='planoarq:settings-lock:v1'||k==='planoarq:preferences:v1'||k===RECOVERY_LAST_KEY||k.startsWith(RECOVERY_PREFIX)||k.startsWith('planoarq:supabase-')||k.startsWith('planoarq:sync-')||k.startsWith('planoarq:drive-')||k.startsWith('planoarq:last-drive')||
...
function buildBackup(scope='all',cid=''){const data=collect(scope,cid);return {schema:'planoarq-backup-v2',version:2,exportedAt:iso(),scope,contestId:scope==='contest'?cid:null,device:{id:deviceId(),name:deviceName()},app:{version:RELEASE,origin:location.origin||'file://'},data,summary:{keys:Object.keys(data).length,bytes:byteSize(data)}}}
function recoveryPayload(id){return safeJSON(localStorage.getItem(RECOVERY_PREFIX+id),null)}
function listRecoveryPoints(){const rows=[];for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(!k?.startsWith(RECOVERY_PREFIX))continue;const p=safeJSON(localStorage.getItem(k),null);if(p?.id&&p?.schema==='planoarq-recovery-v1')rows.push(p)}return rows.sort((a,b)=>Date.parse(b.createdAt||0)-Date.parse(a.createdAt||0))}
function pruneRecoveryPoints(limit=RECOVERY_LIMIT){const rows=listRecoveryPoints();rows.slice(Math.max(1,Number(limit)||RECOVERY_LIMIT)).forEach(p=>localStorage.removeItem(RECOVERY_PREFIX+p.id));return Math.min(rows.length,Math.max(1,Number(limit)||RECOVERY_LIMIT))}
function createRecoveryPoint(reason='manual',affectedKeys=null,meta={}){const source=Array.isArray(affectedKeys)&&affectedKeys.length?[...new Set(affectedKeys)].filter(tracked):keys(),data={},missing=[];source.forEach(k=>{const v=localStorage.getItem(k);if(v===null)missing.push(k);else data[k]=v});const id=Date.now().toString(36)+'-'+uuid().replace(/[^a-z0-9-]/gi,'').slice(-12),payload={schema:'planoarq-recovery-v1',version:1,id,createdAt:iso(),reason:String(reason||'manual'),data,missing,summary:{keys:source.length,bytes:byteSize(data)},meta:meta&&typeof meta==='object'?meta:{}};try{localStorage.setItem(RECOVERY_PREFIX+id,JSON.stringify(payload));localStorage.setItem(RECOVERY_LAST_KEY,id);pruneRecoveryPoints()}catch(err){try{localStorage.removeItem(RECOVERY_PREFIX+id)}catch(_){}throw new Error('Não foi possível criar o ponto de recuperação local. Libere espaço antes de continuar.')}return payload}
function lastRecoveryPoint(){const id=localStorage.getItem(RECOVERY_LAST_KEY)||'';return id?recoveryPayload(id):(listRecoveryPoints()[0]||null)}
function restoreRecoveryPoint(id=''){const target=id?recoveryPayload(id):lastRecoveryPoint();if(!target)throw new Error('Nenhum ponto de recuperação disponível');const affected=[...new Set([...Object.keys(target.data||{}),...(target.missing||[])])].filter(tracked);createRecoveryPoint('before-rollback',affected,{restores:target.id});Object.entries(target.data||{}).forEach(([k,v])=>{if(tracked(k))localStorage.setItem(k,String(v))});(target.missing||[]).filter(tracked).forEach(k=>localStorage.removeItem(k));localStorage.setItem('planoarq:last-backup-import',iso());return {id:target.id,entries:affected.length,reason:target.reason||''}}
...
function importEntries(entries,opts={}){const clean=(Array.isArray(entries)?entries:[]).filter(([k])=>tracked(k));if(!clean.length)return 0;if(opts.recovery!==false)createRecoveryPoint(opts.reason||'backup-import',clean.map(([k])=>k),{source:opts.source||'import',...(opts.meta&&typeof opts.meta==='object'?opts.meta:{})});clean.forEach(([k,v])=>localStorage.setItem(k,String(v)));localStorage.setItem('planoarq:last-backup-import',iso());return clean.length}
function dataHealth(){let parseErrors=0,jsonKeys=0;keys().forEach(k=>{const v=localStorage.getItem(k);if(v&&(/^[\[{]/.test(v.trim()))){jsonKeys++;try{JSON.parse(v)}catch(_){parseErrors++}}});const all=collect(),recoveries=listRecoveryPoints(),lastRecovery=lastRecoveryPoint();return {keys:Object.keys(all).length,bytes:byteSize(all),parseErrors,jsonKeys,lastExport:localStorage.getItem('planoarq:last-backup-export')||'',lastImport:localStorage.getItem('planoarq:last-backup-import')||'',recoveries:recoveries.length,lastRecovery:lastRecovery?{id:lastRecovery.id,createdAt:lastRecovery.createdAt,reason:lastRecovery.reason,keys:lastRecovery.summary?.keys||0}:null}}
